...
For this, you can use the [`run()` function's](/write-code/reference/query-object)  `params` argument to pass an object of key-value pairs into your query. You can access these values within the query using `{{ this.params.key }}`.
...
1. Create a function that calls your query's [`run()` function](/write-code/reference/query-object) in a loop, using the `run()` function's first argument to pass your parameter. On completion, the results of the queries are mapped into an array and stored in the [Appsmith store](/write-code/reference/widget-actions/store-value).
