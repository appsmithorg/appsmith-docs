---
sidebar_position: 3
description: This page provides you common deployment errors and troubleshooting instructions to fix them.
---
...
1. Access your Appsmith instance and make the below change to the `docker-compose.yml` file:
  ...
  <Tabs groupId="appsmithEditions" queryString="current-edition">
    <TabItem value="communityEdition" label="Community Edition" default>
    ...
    <TabItem value="commercialEdition" label="Commercial Edition">
...
## Unable to build Kubernetes objects from current release

This issue occurs while upgrading your Appsmith Business or Enterprise Helm charts from version `3.0.4` to `3.0.7` and results in an upgrade failure.
...
The issue happens because of how Helm handles the upgrade. During an upgrade, Helm compares your existing application configuration with the new chart, verifying the updated/supported API versions. When Kubernetes removes an API version, the Go Client library can no longer understand the deprecated objects. This results in an error when Helm interacts with the library, causing the upgrade to fail.

#### Solution

To resolve this issue, use the [helm-mapkubeapis](https://github.com/helm/helm-mapkubeapis) plugin. This plugin helps Helm understand the changes made in Kubernetes.

Follow these steps to solve the issue:

- Install the `helm-mapkubeapis` plugin with:
  ```bash
  helm plugin install https://github.com/helm/helm-mapkubeapis
  ```
- Verify the plugin installation with:
  ```bash
  helm plugin ls
  ```
- Apply the plugin to the Appsmith Helm chart with (replace `<APPSMITH-CHART_NAME>` and `<NAMESPACE>` with the chart name and the your namespace):
  ```bash
  helm mapkubeapis <APPSMITH-CHART-NAME> -n <NAMESPACE>
  ```
- Update the Appsmith installation with:
  ```bash
