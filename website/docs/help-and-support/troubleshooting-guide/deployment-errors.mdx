...
## Server keeps shutting down with schema mismatch error

If your server keeps shutting down and displays a schema mismatch error, it may be due to a missed checkpoint upgrade. 

#### Error message

```bash
Looks like you skipped some required update(s), please go back to the mandatory upgrade path v1.9, or refer to ''https://docs.appsmith.com/'' for more info
```

#### Cause
As the error message suggests that your Appsmith instance has missed a required checkpoint upgrade. This could have occurred if you are manually upgrading and have skipped a version, or if you are on automatic upgrade but your instance hasn't checked for updates for a certain period of time.

#### Solution

To fix this error, you may need to manually set your instance to the required version, `v1.9`. To do this, access your Appsmith installation instance and make the following change to the `docker-compose.yml` file:
...
services:
  appsmith:
    image: index.docker.io/appsmith/appsmith-ce:v1.9.0
    container_name: appsmith
```
...
services:
  appsmith:
    image: index.docker.io/appsmith/appsmith-ee:v1.9.0
    container_name: appsmith
```
...
With this your server starts with the older version that includes all necessary schema changes for your Appsmith instance. Once this process has completed successfully (you can verify this by attempting to log in to your Appsmith instance), you can return to the original version of the `docker-compose.yml` file and resume normal operations.
...
Now, run the restart command again: 

 ```
 docker-compose up -d
  ```

To prevent similar issues in the future, consider enabling auto updates for your Appsmith instance. If you already had auto updates on and still encountered this issue, please contact the [Appsmith team](https://discord.com/invite/rBTTVJp) to determine the cause of the deployment problem