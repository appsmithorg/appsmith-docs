...
- Integrate Appsmith into the Azure Virtual Network (vnet). Verify that your Appsmith instance is part of the same Azure virtual network as your PostgreSQL database.

- Add a private endpoint for your PostgreSQL server within the same vnet. Confirm that this setup allows the Appsmith instance to connect to the database through private network routing.

- Test the connection after making these changes to ensure that the Appsmith instance can successfully connect to the PostgreSQL database.
