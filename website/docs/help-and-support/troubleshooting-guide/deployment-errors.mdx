...
If you are trying to connect to an existing [MongoDB](../../data/datasource-reference/querying-mongodb/) and the containers failed to start it may be due to one of the following reasons:

1. Incorrect [MongoDB](../../data/datasource-reference/querying-mongodb/) credentials
...
This ensures that the redirect URLs are correct during [OAuth2](/data/datasource-reference/authenticated-api) logins. This works even if the ELB is configured to run on a custom port.

## Server not booting because of MongoCommandException

In release `v1.6.4`, Appsmith upgraded its libraries & [Spring framework](https://spring.io/projects/spring-framework). This caused a compatibility issue between the libraries used within Appsmith and the version of [MongoDB](/data/datasource-reference/querying-mongodb/) that was shipped earlier. This didn't show up in testing because all testing happened against [MongoDB](/data/datasource-reference/querying-mongodb/) clusters with replica sets, where the problem doesn't surface.
