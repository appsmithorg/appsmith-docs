...
## Error auto updating Docker containers using Watchtower 
...
#### Cause
The Watchtower may encounter errors when attempting to update the Docker containers. The `exec format error` usually occurs when trying to run an image on an incompatible CPU architecture. 
...
After making the necessary changes, run the command ```docker-compose up -d``` again to start the containers.

If you still encounter this issue, please contact the [Appsmith team](https://discord.com/invite/rBTTVJp) to determine the cause of the deployment problem
