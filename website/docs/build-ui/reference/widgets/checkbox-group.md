...
This event defines the action that would be executed when the user selects or deselects multiple or single items in the checkbox group. It allows you to specify a list of [supported actions](/write-code/reference/widget-actions) that can be triggered in response to the checkbox state change.
