...
This event defines the action that would be executed when the user selects or deselects multiple or single options in the switch group. It allows you to specify a list of [actions](/write-code/reference/widget-actions) that can be triggered in response to the switch state change.
