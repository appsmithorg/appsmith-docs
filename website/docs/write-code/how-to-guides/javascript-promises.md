...
All Appsmith framework functions like [showAlert()](/write-code/reference/widget-actions/show-alert.md), [showModal()](/write-code/reference/widget-actions/show-modal.md), [storeValue()](/write-code/reference/widget-actions/store-value.md), and the others returns a promise, making asynchronous workflows implementation easier and readable.
...
## Async/Await
The `async` and `await` keywords enable the [asynchronous](/write-code/concepts/javascript-editor-beta#asynchronous) workflow to be written in a cleaner style, avoiding the need to configure promise chains explicitly.
