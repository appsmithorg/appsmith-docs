---
description: Permissions in Granular Access Control in Appsmith
title: Permissions
hide_title: true
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<!-- vale off -->

<div className="tag-wrapper">
 <h1>Permissions</h1>

<Tags
tags={[
{ name: "Business", link: "https://www.appsmith.com/pricing", additionalClass: "business" }
]}
/>

</div>

<!-- vale on -->

Permissions in Appsmith govern the level of access and actions users can perform on specific resources within the platform. To assign appropriate permissions to resources, so users can efficiently complete their tasks, it's crucial to understand the permissions, their interdependencies, and their cascading impact. This page provides an in-depth overview of permissions, their assignments, and interdependencies within the Appsmith access control system.

## Create permission

The **Create** permission permits users to create new resources in Appsmith. When the **Create** permission is assigned to a parent resource, it, along with the automatically assigned permissions, are inherited by the child resources.

<Tabs queryString="gac-resource">
  <TabItem label="Application Resources" value="create-application-resources">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Create also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>Edit, View, Delete, Execute</td>
          <td>Allows creating, editing, viewing, and deleting applications, pages, and queries. It also permits running queries within the given workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Application</td>
          <td>Edit, View, Delete, Execute</td>
          <td>Allows creating, editing, viewing, and deleting pages and queries. It also permits running queries within the given application.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Page</td>
          <td>Edit, View, Delete, Execute</td>
          <td>Allows creating, editing, viewing, deleting, and running queries within the given application.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Datasources & Environments" value="create-datasources-and-environments">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Create also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Datasources</b></td>
          <td>Edit, View, Delete, Execute</td>
          <td>Allows creating, editing, viewing, and deleting datasources. It also grants permission to run queries in all datasources within the workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Datasource Name</td>
          <td>Edit, View, Delete, Execute</td>
          <td>Allows creating, editing, viewing, and deleting the specified datasource. It also grants permission to run queries in the given datasource.</td>
        </tr>
        <tr>
          <td><b>Environments</b></td>
          <td>Edit, View, Delete, Execute</td>
          <td>Allows creating, editing, viewing, and deleting environments. It also grants permission to run queries in all environments (Production, Staging, and custom created) within the workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Environment</td>
          <td>Edit, View, Delete, Execute</td>
          <td>Allows creating, editing, viewing, and deleting the specified environment. It also grants permission to run queries in the given environment (Production, Staging, or custom created) within the workspace.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Workflows" value="create-workflows">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Create also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>Edit, Delete</td>
          <td>Allows adding new workflows, editing and deleting existing workflows within the Appsmith workspace.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Groups & Roles" value="create-groups-and-roles">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Create also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Groups</b></td>
          <td>Edit, View, Delete, Invite User, Remove User</td>
          <td>Allows creating, editing, viewing, and deleting groups. It also allows inviting users to the instance or removing users from the instance.</td>
        </tr>
        <tr>
          <td><b>Roles</b></td>
          <td>Edit, View, Delete, Associate Role</td>
          <td>Allows creating, editing, viewing, and deleting custom roles. It also allows viewing default roles and assigning custom and default roles to users or groups.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Custom Roles</td>
          <td>Edit, View, Delete, Associate Role</td>
          <td>Allows creating, editing, viewing, and deleting the specified custom role. It also allows assigning the given custom role to users or groups.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Others" value="create-others">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Create also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspaces</b></td>
          <td>NA</td>
          <td>When assigned to workspaces, it allows adding new workspaces to the Appsmith instance.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## Edit permission

The **Edit** permission defines permits users to edit existing resources in Appsmith. When the **Edit** permission is assigned to a parent resource, it, along with the automatically assigned permissions, are inherited by the child resources.

<Tabs queryString="gac-resource">
  <TabItem label="Application Resources" value="edit-application-resources">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Edit also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>View, Execute</td>
          <td>Allows editing, viewing, and running queries within the given workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Application</td>
          <td>View, Execute</td>
          <td>Allows editing, viewing, and running queries within the given application.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Page</td>
          <td>View, Execute</td>
          <td>Allows editing, viewing, and running queries within the given application.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Datasources & Environments" value="edit-datasources-and-environments">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Edit also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Datasources</b></td>
          <td>View, Execute</td>
          <td>Allows editing, viewing, and running queries in all datasources within the workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Datasource Name</td>
          <td>View, Execute</td>
          <td>Allows editing, viewing, and running queries in the given datasource.</td>
        </tr>
        <tr>
          <td><b>Environments</b></td>
          <td>View, Execute</td>
          <td>Allows editing, viewing, and running queries in all environments within the workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Environment</td>
          <td>View, Execute</td>
          <td>Allows editing, viewing, and running queries in the given environment within the workspace.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Workflows" value="edit-workflows">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Edit also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>NA</td>
          <td>Allows editing existing workflows within the Appsmith workspace.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Groups & Roles" value="edit-groups-and-roles">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Edit also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Groups</b></td>
          <td>View, Invite User, Remove User</td>
          <td>Allows editing, viewing, inviting users to, and removing users from the instance.</td>
        </tr>
        <tr>
          <td><b>Roles</b></td>
          <td>View, Associate Role</td>
          <td>Allows editing, viewing, and assigning custom and default roles to users or groups.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Custom Roles</td>
          <td>View, Associate Role</td>
          <td>Allows editing, viewing, and assigning the given custom role to users or groups.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## Delete permission

The **Delete** permission defines permits users to delete existing resources in Appsmith. When the **Delete** permission is assigned to a parent resource, it, along with the automatically assigned permissions, are inherited by the child resources.

<Tabs queryString="gac-resource">
  <TabItem label="Application Resources" value="delete-application-resources">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Delete also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>View, Execute</td>
          <td>Allows deleting, viewing, and running queries within the given workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Application</td>
          <td>View, Execute</td>
          <td>Allows deleting, viewing, and running queries within the given application.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Page</td>
          <td>View, Execute</td>
          <td>Allows deleting, viewing, and running queries within the given application.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Datasources & Environments" value="delete-datasources-and-environments">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Delete also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Datasources</b></td>
          <td>View, Execute</td>
          <td>Allows deleting, viewing, and running queries in all datasources within the workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Datasource Name</td>
          <td>View, Execute</td>
          <td>Allows deleting, viewing, and running queries in the given datasource.</td>
        </tr>
        <tr>
          <td><b>Environments</b></td>
          <td>View, Execute</td>
          <td>Allows deleting, viewing, and running queries in all environments within the workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Environment</td>
          <td>View, Execute</td>
          <td>Allows deleting, viewing, and running queries in the given environment within the workspace.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Workflows" value="delete-workflows">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Delete also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>NA</td>
          <td>Allows deleting existing workflows within the Appsmith workspace.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Groups & Roles" value="delete-groups-and-roles">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Delete also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Groups</b></td>
          <td>View</td>
          <td>Allows deleting and viewing groups within the instance.</td>
        </tr>
        <tr>
          <td><b>Roles</b></td>
          <td>View, Associate Role</td>
          <td>Allows deleting, viewing, and assigning custom and default roles to users or groups.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Custom Roles</td>
          <td>View, Associate Role</td>
          <td>Allows deleting, viewing, and assigning the given custom role to users or groups.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## View permission

The **View** permission permits users to view existing resources in Appsmith. When the **View** permission is assigned to a parent resource, it, along with the automatically assigned permissions, are inherited by the child resources.

<Tabs queryString="gac-resource">
  <TabItem label="Application Resources" value="view-application-resources">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>View also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>Execute</td>
          <td>Allows viewing and running queries within the given workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Application</td>
          <td>Execute</td>
          <td>Allows viewing and running queries within the given application.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Page</td>
          <td>Execute</td>
          <td>Allows viewing and running queries within the given application.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Datasources & Environments" value="view-datasources-and-environments">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>View also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Datasources</b></td>
          <td>Execute</td>
          <td>Allows viewing and running queries in all datasources within the workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Datasource Name</td>
          <td>Execute</td>
          <td>Allows viewing and running queries in the given datasource.</td>
        </tr>
        <tr>
          <td><b>Environments</b></td>
          <td>Execute</td>
          <td>Allows viewing and running queries in all environments within the workspace.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Environment</td>
          <td>Execute</td>
          <td>Allows viewing and running queries in the given environment within the workspace.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Groups & Roles" value="view-groups-and-roles">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>View also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Groups</b></td>
          <td>NA</td>
          <td>Allows viewing groups within the instance.</td>
        </tr>
        <tr>
          <td><b>Roles</b></td>
          <td>Associate Role</td>
          <td>Allows viewing and assigning custom and default roles to users or groups.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Default Roles</td>
          <td>Associate Role</td>
          <td>Allows viewing and assigning the default role to users or groups.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Custom Roles</td>
          <td>Associate Role</td>
          <td>Allows viewing and assigning the given custom role to users or groups.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Others" value="view-others">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>View also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Audit logs</b></td>
          <td>NA</td>
          <td>Allows viewing the Appsmith instance audit logs that helps in monitoring the instance.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## Execute permission

The **Execute** permission permits users to execute actions such as running queries in Appsmith. When the **Execute** permission is assigned to a parent resource, it's also inherited by the child resources.

<Tabs queryString="gac-resource">
  <TabItem label="Application Resources" value="execute-application-resources">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Execute also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>NA</td>
          <td>Allows running queries within the given workspace.</td>
        </tr>
        <tr>
          <td><b>Application</b></td>
          <td>NA</td>
          <td>Allows running queries within the given application.</td>
        </tr>
        <tr>
          <td><b>Page</b></td>
          <td>NA</td>
          <td>Allows running queries within the given application.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
  <TabItem label="Datasources & Environments" value="execute-datasources-and-environments">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Execute also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Datasources</b></td>
          <td>NA</td>
          <td>Allows running queries in all datasources within the workspace.</td>
        </tr>
        <tr>
          <td><b>Datasource</b></td>
          <td>NA</td>
          <td>Allows running queries in the given datasource.</td>
        </tr>
        <tr>
          <td><b>Environments</b></td>
          <td>NA</td>
          <td>Allows running queries in all environments within the workspace.</td>
        </tr>
        <tr>
          <td><b>Environment</b></td>
          <td>NA</td>
          <td>Allows running queries in the given environment within the workspace.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## Make Public permission

The **Make Public** permission permits users to change the access level of resources, making them available to the public. When the **Make Public** permission is assigned to a workspace or an application, it, along with the automatically assigned permissions, are inherited by the child resources.

<Tabs queryString="gac-resource">
  <TabItem label="Application Resources" value="make-public-application-resources">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Make Public also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>View, Execute</td>
          <td>Allows making all applications publicly accessible within the given workspace, and allows viewing and running queries to view the application when applications are publicly accessible.</td>
        </tr>
        <tr>
          <td><b>Application</b></td>
          <td>View, Execute</td>
          <td>Allows making the given application publicly accessible, and allows viewing and running queries to view the application when it's publicly accessible.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## Export permission

The **Export** permission permits users to export data and resources from Appsmith. When the **Export** permission is assigned to a workspace or an application, it, along with the automatically assigned permissions, are inherited by the child resources.

<Tabs queryString="gac-resource">
  <TabItem label="Application Resources" value="execute-application-resources">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Export also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Workspace</b></td>
          <td>View, Execute</td>
          <td>Allows viewing application, exporting data and resources, and running queries required to view application and exporting data within the given workspace.</td>
        </tr>
        <tr>
          <td><b>Application</b></td>
          <td>View, Execute</td>
          <td>Allows viewing application, exporting data and resources, and running queries required to view application and exporting data within the given application.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## Invite User permission

The Invite User permission it permits users to invite other users to Appsmith workspace or applications and provides access to the resources within workspace.

<Tabs queryString="gac-resource">
  <TabItem label="Groups & Roles" value="invite-user-groups-and-roles">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Invite User also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Groups</b></td>
          <td>View</td>
          <td>Allows viewing the groups, and inviting users to workspaces and applications within the instance.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## Remove User permission

The Remove User permission permits users to remove other users from the resources in Appsmith.

<Tabs queryString="gac-resource">
  <TabItem label="Groups & Roles" value="remove-user-groups-and-roles">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Remove User also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Groups</b></td>
          <td>View, Invite User</td>
          <td>Allows viewing groups, inviting users to workspaces, and applications, and removing users from groups within the instance that helps removing their access to the instance.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>

## Associate Role permission

The Associate Role permission permits users to assign or change roles of other users in Appsmith.

<Tabs queryString="gac-resource">
  <TabItem label="Groups & Roles" value="associate-role-groups-and-roles">
    <table className="permissions-table">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Associate Role also grants</th>
          <th>Permission Impact</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td><b>Roles</b></td>
          <td>NA</td>
          <td>Allows assigning default and custom roles to users within the instance.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Default Roles</td>
          <td>NA</td>
          <td>Allows assigning the given custom role to users within the instance.</td>
        </tr>
        <tr>
          <td>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;Custom Roles</td>
          <td>NA</td>
          <td>Allows assigning the given custom role to users within the instance.</td>
        </tr>
      </tbody>
    </table>
  </TabItem>
</Tabs>