...
- Access to edit role configuration and applications.
...
To allow only specific users to execute a query, identify the query that processes sensitive data (e.g., *query_process_sensitive_data*) and the roles that need access to execute the query, as well as roles that no longer need this permission. Then update the role configuration as follows:
...
4. Click the pencil icon next to the role to edit its permissions.
