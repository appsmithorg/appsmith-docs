...
Finally, click on **connect**, and when the connection is successful, your repository will have a `README.md` file. On the Git modal, you’ll move to the **Deploy** tab, where you can make an initial commit.
...
If you are a self-hosted user, Appsmith clones the git repositories in the local filesystem, attached to the persistent volume within the docker container. To maintain the git repositories, you need a file path that points to the volume within the docker container.

To update the local filepath to  point to a custom Git Root where the git repositories is persisted, update the Appsmith Git Root environment variable called `APPSMITH_GIT_ROOT` to point to your custom file path.
