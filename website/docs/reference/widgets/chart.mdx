...
* **Type :** The type is used to define the chart type recognized by the FusionCharts, written in JavaScript. FusionCharts provides an extensive range of chart variants, with almost 100+ options to choose from. For example: `"type": "column2d"`

* **Datasource :** The datasource defines the customization options and data points necessary for creating a chart, written in JSON format. It consists of attributes such as chart and data, which may vary depending on the chart type.
