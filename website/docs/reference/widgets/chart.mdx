...
**Example**, suppose you have a database containing information about assets in your organization, such as their types and the date they were purchased. You can create a query `getAssetsByType` that groups the assets by type and count how many there are of each type, as shown below:
```sql
...
**Example**, suppose you have business revenue data fetched from an API `getRevenue` and want to display it in a chart. You can transform the data using the `map()` JS function as shown below:
