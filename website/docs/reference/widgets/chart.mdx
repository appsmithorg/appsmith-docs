...
**Chart Type** `String`
  ...
  Sets the type of Chart used to display data. Choose one of the charts from the available options, or create a custom chart (available from FusionCharts).
    
**Series Data** `String`

  Sets the type of Chart used to display data. Choose one of the charts from the available options, or create a custom chart (available from FusionCharts).
    
Specifies the data points for the current chart series, providing the values to be plotted on the chart. 

Example:

The series data must be in a specific format.
```js
[
  {
    "x": "Product1",
