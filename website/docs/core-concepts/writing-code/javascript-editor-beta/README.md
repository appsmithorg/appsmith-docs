...
The JavaScript Editor automatically checks your source code for programmatic errors. If the code isn't programmatically correct, it will highlight the error using a red-colored lint below the erroneous code. For example, the syntax error where the `return` is misspelled as `retu` is also captured by linting.
...
You can use the debugger statement to pause the execution of your code and inspect the code in a browser developer console. This allows you to inspect the state of your code at that point and step through it line by line to help identify and fix any errors. 

:::note
You must have the browser developer console open to see the debugger in action.
:::
