...
1. Create a function using one of the following code snippets:
    ...
    * The code snippets above run the query once for each `id` in its `ids` array, wait for the database's response, and store the resulting table records in the [Appsmith store](/reference/appsmith-framework/widget-actions/store-value) for you to access later.
...
When you click the button, your helper function runs and queries the database for each of your desired users.