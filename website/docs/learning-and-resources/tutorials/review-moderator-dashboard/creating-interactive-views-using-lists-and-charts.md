...
3\. Lets add a few text widgets to display the business name and the business ID. The below query can be used to add a name. You can use a business id or any other value in place of a name.
...
Now lets display all the reviews in the [list view](/reference/widgets/list). List widget provides you with out-of-the-box feature to iterate over a structured data collection. The datasets can be static or generated by the response from API/queries.
...
Now Lets make the list view even more intuitive. Drag and drop a text widget onto the table, and add the following code snippet in the Text property from its property pane:
...
Lets display the reviews in a colorful way. Add a few text widgets inside the list view.
...
Lets deploy your app for the final time. Once deployed, you can share your deployed application with both internal and external users:
