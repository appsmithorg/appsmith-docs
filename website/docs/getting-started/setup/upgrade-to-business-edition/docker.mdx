...
The Community and Business editions use Mongo DB, so you don't have to migrate the database. Though, it's recommended to [backup the Community Edition Mongo DB](/getting-started/setup/instance-management/appsmithctl#export-database).
...
Once the container is up and running, you have successfully upgraded the docker instance to the Business Edition. You can verify that the business image (_appsmith-ee_) is in use with the help of the **`docker ps`** command.
...
* Navigate to the `docker-compose.yml` file, and look for the `image:` key. Update the image in the `docker-compose.yml` file from Business Edition (_appsmith-ee_) to Community Edition (_appsmith-ce_). 
...
* Recreate the instance by running the below command:

```bash
docker-compose up -d
```
Once the container is up and running, you have successfully upgraded the docker instance to the Business Edition. You can verify that the business image (_appsmith-ce_) is in use with the help of the **`docker ps`** command.
