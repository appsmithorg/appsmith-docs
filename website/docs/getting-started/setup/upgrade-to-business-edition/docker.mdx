...
The Community and Business editions use Mongo DB, so you don't have to migrate the database. TIt's still recommended to [backup the Community Edition Mongo DB](/getting-started/setup/instance-management/appsmithctl#export-database).
...
* Go to the directory (`~stacks\configuration`) and open the `docker.env`. Remove the license key (APPSMITH\_LICENSE\_KEY)
* Go to the `docker-compose.yml` file, and look for the `image:` key. Update the image in the `docker-compose.yml` file from Business Edition (_appsmith-ee_) to Community Edition (_appsmith-ce_). 
