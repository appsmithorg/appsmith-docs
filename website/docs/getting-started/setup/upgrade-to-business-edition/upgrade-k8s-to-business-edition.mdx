...
<li> Run the following commands to backup the database which is available at <code>/appsmith-stacks/connect-data/backup</code>
...
```bash
# Download the backup file 
kubectl cp ./<name_of_backup_file> <namespace>/<pod>:/appsmith-stacks/connect-data/backup/
