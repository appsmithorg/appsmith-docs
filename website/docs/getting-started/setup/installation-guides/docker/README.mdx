---
description: The guide provides instructions to install Appsmith using Docker.
---

# Docker

This page provides instructions to install Appsmith using Docker.

<VideoEmbed
  host="youtube"
  videoId="Tde7GqE6FQQ"
  title="How to install Appsmith using Docker"
  caption="How to install Appsmith using Docker Compose"
/>{" "}

## Prerequisites

Before you begin, ensure you have the following prerequisites:

1. [Docker](https://docs.docker.com/get-docker/) (version 20.10.7 or later)
2. [Docker-Compose](https://docs.docker.com/compose/install/) (version 1.29.2 or later)

## Install Appsmith

Follow these steps to install Appsmith:

1. Create a folder named `appsmith` on your machine for deployment and data storage. Navigate to this folder using the `cd` command.

2. Create a `docker-compose.yml` file with:

   ```yaml
   version: "3"
   services:
      appsmith:
        image: index.docker.io/appsmith/appsmith-ee
        container_name: appsmith
        ports:
            - "80:80"
            - "443:443"
        volumes:
            - ./stacks:/appsmith-stacks
        restart: unless-stopped
   ```

   The `appsmith-ee` image installs the _Commercial edition_ (recommended), which offers a _free plan_ and the flexibility to upgrade to a paid plan at any point. To install the _Community edition_, replace `appsmith-ee` with `appsmith-ce` in the image attribute within the `docker-compose.yml` file.

3. Start the Docker container by using the below command. You may need to use `sudo` if you don't have permission to run `docker-compose`:

   ```bash
   docker-compose up -d
   ```

   This command will download the necessary Docker image if it doesn't exist locally and start the container.

4. Open [http://localhost](http://localhost) and wait for the server to come up. This can take up to 5 minutes. Once the server is up and running, you can access Appsmith at [http://localhost](http://localhost).

5. Fill in your details to create an administrator account.

6. Once you've created an account, you can either start with the free plan or activate your instance with a license key. If you want to generate a license key, sign up on [customer.appsmith.com](https://customer.appsmith.com) to create one, and then proceed to activate your instance using the newly generated license key.


## Post-installation set up

Once you have completed the installation process, there are specific steps you may want to take to configure or manage to enhance the security and performance of your Appsmith instance, specially if it's intended for production use. 

<div className="containerGridSampleApp">
<div className="containerColumnSampleApp columnGrid column-one">
        <div className="containerCol">
            <a href="/getting-started/setup/instance-configuration/custom-domain"><strong>Custom Domain and SSL</strong></a>
        </div> <hr/>
        <div className="containerDescription">This page shows how to set up SSL for your custom domain on the Appsmith instance.</div>
        <div className="containerTutorialLink"></div>
    </div>
<div className="containerColumnSampleApp columnGrid column-two">
<div className="containerCol">
            <a href="/getting-started/setup/instance-configuration/email"><strong>Email Service</strong></a>
        </div> <hr/>
        <div className="containerDescription">This page shows you how to configure an email service provider on your Appsmith instance.</div>
</div>
</div>


<div className="containerGridSampleApp">
<div className="containerColumnSampleApp columnGrid column-one">
        <div className="containerCol">
            <a href="/getting-started/setup/instance-configuration/custom-domain"><strong>Schedule Automatic Updates</strong></a>
        </div> <hr/>
        <div className="containerDescription">This page shows you how to manage your instance by scheduling automatic updates using cron expression.</div>
        <div className="containerTutorialLink"></div>
    </div>
<div className="containerColumnSampleApp columnGrid column-two">
<div className="containerCol">
            <a href="/getting-started/setup/instance-configuration/email"><strong>Backup and Restore</strong></a>
        </div> <hr/>
        <div className="containerDescription">This page shows you how to manage your instance by using the backup and restore commands.</div>
</div>
</div>

## Troubleshooting

If you are facing issues during deployment, please refer to the guide on [troubleshooting deployment errors](/help-and-support/troubleshooting-guide/deployment-errors).

If you continue to face issues, contact the support team using the chat widget at the bottom right of this page.