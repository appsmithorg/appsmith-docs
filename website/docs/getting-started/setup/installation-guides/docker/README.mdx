...
Once the Appsmith container is ready, the message "Appsmith is Running" would be displayed in the Docker logs.
...
This message is also logged and available in server logs in (`stacks/logs/backend/backend.log`). You can access backend logs from your Appsmith deployment by following these steps:
...
You can either share these logs with the Appsmith team or retain them for your own reference.
