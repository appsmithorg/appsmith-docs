...
3. Add the following environment variables:
    ...
    - `APPSMITH_REDIS_URL=rediss://:<auth-string>@<host>:6379`: Use the Memorystore primary endpoint with the AUTH string so Appsmith connects with authentication. Use the `rediss://` scheme because in-transit encryption is enabled. See [Enable authentication](/getting-started/setup/instance-configuration/external-redis#enable-authentication).
