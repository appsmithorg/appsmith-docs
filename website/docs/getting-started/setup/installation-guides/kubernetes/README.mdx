...
:::
  ...
  3. PostgreSQL has to be enabled Or configure an external PostgreSQL in the `applicationConfig` section in `values.yaml`.
      ...
      i. Configure the parameters to enable PostgreSQL as shown below:
      ...
      ii. Configure the parameters to point to an external PostgreSQL as shown below:
  ...
  4.  Enable MongoDB in the `values.yaml` or configure an external MongoDB in the `applicationConfig` in `values.yaml`.
    ...
    i. Configure the parameters to enable MongoDB as shown below:
     ...
     ii. Configure the parameters to point to an external MongoDB as shown below:
...
5. Run the below command to deploy Appsmith:
 ...
 i. To get the application URL run the below commands:
...
ii. Run the command `kubectl get pods` to verify the list of pods. You may see output as shown below:
...
To install the Nginx ingress controller on AWS-EKS, GCE-GKE, or Azure, run the following command if it hasn't already been installed:
  ...
  * To configure using `values.yaml`.
...
* It takes a few minutes for the ingress to get an IP assigned. Once it has been assigned, you should see an IPV4 address or a FQDN in the ADDRESS column. Run the below command to verify the assigned IP address/FQDN:
