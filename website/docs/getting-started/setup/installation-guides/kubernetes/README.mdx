...
3. Generate the `values.yaml` config file
    ...
    ```
4. Edit the `values.yaml` file and set the following parameters
...
<details id="highAvailability" open>
  ...
  2.  Enable `autoscaling` in the `values.yaml` (This specifies the autoscaling config for the Appsmith Deployment):
   ...
   ```
  :::note
  When autoscaling is enabled both the postgresql and mongodb will also need to be enabled, or configured to point external instance.
  :::

  3. PostgreSQL has to be enabled 
      ```yaml
            postgresql:
            # highlight-next-line
                enabled: true
                auth:
                  username: root
      ```
    Or provison an external PostgreSQL and update the following data values of ApplicationConfig in `values.yaml`.
      ```yaml
        applicationConfig::
        ....
          APPSMITH_KEYCLOAK_DB_DRIVER: "postgresql"
          APPSMITH_KEYCLOAK_DB_USERNAME: "<postgres user>"
          APPSMITH_KEYCLOAK_DB_PASSWORD: "<postgres password>"
          APPSMITH_KEYCLOAK_DB_URL: "<postgres db uri>"
        ....
      ```

  4.  Enable `mongodb` in the `values.yaml`,
      ```yaml
        mongodb:
          # highlight-next-line
          enabled: true
      ...
      ```
     Or provision an external mongodb and update following data values of ApplicationConfig in `values.yaml`.
       ```yaml
        applicationConfig::
        ....
          APPSMITH_MONGODB_URI: "<mongodb uri>"
        ....
      ```
...
Please refer to the instructions [here](/getting-started/setup/installation-guides/kubernetes#publish-appsmith) to publish you Appsmith installation on the internet.
...
1. Run the command below to install the nginx ingress controller ( For AWS-EKS, GCE-GKE or Azure)
  ```bash 
  helm install ingress-nginx ingress-nginx/ingress-nginx
  ```
2. Enable the ingress controller by either:
    - Running the below command.
        ```bash
        helm upgrade [appsmith-ee|appsmith]/appsmith \
            --set ingress.enabled=true \
            --set ingress.className=nginx \
            --set service.type=ClusterIP \
        ```
    - Or edit the `values.yaml` and specify the following parameters
      ```yaml 
      ingress:
  ...
  3. Verify the IP address/FQDN is set by running
  ```bash
  kubectl get ingress
  ```
  It takes a few minutes for the ingress to get an IP assigned. Once it has been assigned, you should see an IPV4 address or a FQDN in the ADDRESS column.
  eg:
  ```bash
  ❯ kubectl get ingress
