...
## Prerequisites
Before installing Appsmith, a few prerequisites need to be installed and configured.
1. Install the [Helm Package Manager](https://helm.sh/docs/intro/install/)- Helm is a package manager for Kubernetes. It allows you to install and manage applications on Kubernetes. To install Helm, follow the instructions for your platform from the official [Helm documentation](https://helm.sh/docs/intro/install/).
2. Install [`kubectl`](https://kubernetes.io/vi/docs/tasks/tools/install-kubectl/) - `kubectl` is the command-line interface for Kubernetes. It allows you to run commands against Kubernetes clusters to manage applications and other resources. To install kubectl, follow the instructions for your platform from the official [Kubernetes documentation](https://kubernetes.io/vi/docs/tasks/tools/install-kubectl/).

### Configure Kubernetes cluster
Once the prerequisites are installed, configure the Kubernetes cluster by following one of the available guidelines below.
...
You've installed and configured the Kubernetes cluster. Now, you can install Appsmith using Helm.
...
It creates all the necessary Kubernetes objects to run Appsmith on your clusters, such as pods, services, and ingresses with the default configuration. Read more on configurations you can do in the [Configure Appsmith](#configure-appsmith) section.
...
```
When the deployment is ready, you should see a message that says deployment "appsmith" successfully rolled out.

6. Access Appsmith by visiting the URL that was output by the Helm installation command. You can also use the `kubectl` command to get the URL. The URL has a `hostname` and `port` of the Appsmith ingress to access Appsmith using a web browser.
...
## Configure Appsmith
You should be able to use Appsmith with the default configuration. If you wish, you could also configure the installation to fit your specific needs. You could choose to configure parameters using command line, or using `values.yaml` file. Below is the exhaustive list of available parameters that can be used to customize the installation.

### Parameters
Appsmith can be configured using Helm parameters, or parameters specific to Appsmith. The below table details the different Helm and Appsmith specific parameters that you can configure.
...
In case you haven't installed the Helm chart yet, you can run the below command to install helm chart, enable ingress controller, and publish Appsmith.
