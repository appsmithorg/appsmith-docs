...
## Before you begin
1. Installed Helm package manager on your local machine. See the official Helm documentation for your [operating system](https://helm.sh/docs/intro/install/#through-package-managers).
2. Installed and configured `kubectl` to interact with your Kubernetes cluster. Follow the below guides available on the official Kubernetes documentation for instructions on how to install `kubectl` on your specific operating system: 
...
3. A running Kubernetes cluster. If not present, follow these steps to set up the cluster on AWS EKS:
...
<Tabs groupId="appsmithEditions" queryString="current-edition">
  ...
  8. Open [http://localhost:8080](http://localhost:8080) and wait for the server to come up. This can take up to 5 minutes. Once the server is up and running, you can access Appsmith at [http://localhost:8080](http://localhost:8080). To expose Appsmith installation on internet, see the [Publish Appsmith on internet](/getting-started/setup/installation-guides/kubernetes/publish-appsmith-online) guide.
