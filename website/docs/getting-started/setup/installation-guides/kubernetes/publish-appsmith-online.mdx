---
description: Publish Appsmith on a Kubernetes cluster to internet
---
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Publish Appsmith on Internet
The page provides steps to publish Appsmith Kubernetes installation on internet.

## Prerequisites
1. A Kubernetes cluster running Appsmith with at least one node
3. Ingress Controller installed on your Kubernetes cluster. 
...
3. Configuring data compression for Nginx ingress controller can help reduce the amount of data transferred between the server and client, which improves the performance of your application. If you want to use `gzip` compression, follow these steps to enable it.
