...
2. To update the installation of Appsmith with high availability configuration, run the command below. If the `values.yaml` file is not in the deployment directory specify the file location using the `-f` attribute.
