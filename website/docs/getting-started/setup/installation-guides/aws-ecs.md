...
6\. Go to the **Volumes** section and add a new volume. 
    ...
    Option 2 > Amazon EFS volumes. The data is independent of the lifecycle of the host ec2 instance/ecs cluster.
     Prequisite: Create an EFS filesystem by following the steps below:
      ...
      - Click on the EFS created and navigate to the Network tab to ensure mount-target is created in the same availability-zone as taht of the ECS cluster.
