...
To install the Appsmith open source edition (Appsmith Community), replace `appsmith-ee` with `appsmith-ce` while creating a service in step 2 of the [Install Appsmith](#install-appsmith) section on this page.
