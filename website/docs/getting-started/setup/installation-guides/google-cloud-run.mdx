...
2. Enable the [Cloud Filestore API](https://console.cloud.google.com/marketplace/product/google/file.googleapis.com).
...
4. When the new instance creation is complete, note down the **File share name** and **IP address** to use later when mounting the Filestore instance in Cloud Run container.  
...
7. You need to create a Redis instance to manage traffic among revisions
    - Go to [Memorystore](https://console.cloud.google.com/memorystore/redis/instances)
    - Create a Memorystore instance
        - In the instanceid enter `instanceid`
        - Choose **Tier Selection**
            - Basic (No automatic failover)
            - Standard (Automatically failover and High availability)
        - In the **Capacity** box, enter `10`
        - In the **Region** box, enter `region` of choice
        - In the **Zone** box, enter `zone` of choice
        - In the **Read Replicas** box, enter `2 Read Replicas`
        - In the **Set up connection** box, enter `default`
        - In the **Configuration** box, enter `6.x`
    - Once the Memorystore instance is ready make a note of **Primary Endpoint**.

8. Under **Authentication**, select **Allow unauthenticated invocations**.

9. Click **Container, Networking, Security** to expand the service configuration page.

10. Click the **Container** tab.
    ...
    - Specify values in the **Name** and **Value** text boxes as shown below: 
    ...
    | **APPSMITH_MONGODB_URI** | Specify the connection string of the external MongoDB instance required for Appsmith data storage. <br/> <i>Format</i>: <br/>`mongodb://<USERNAME>:<PASSWORD>@<HOST>:<PORT>/?authSource=<DATABASE_NAME>&replicaSet=<REPLICA_SET_NAME>[&options]`|
    | **APPSMITH_ENCRYPTION_SALT** | Specify a encryption salt to encrypt values in database. <br/> <i>Format</i>: <br/>` Encryption Salt`|
    | **APPSMITH_ENCRYPTION_PASSWORD** | Specify a encryption password to encrypt values in database. <br/> <i>Format</i>:<br/>` Encryption Password`|
    | **APPSMITH_REDIS_URL** | Specify the connection string of the external Redis instance required to maintain session and managing traffic. <br/> <i>Format</i>: <br/>`redis://<HOST>:<PORT>`|
...
12. Click the **Security** tab. Note that Cloud Run services or jobs run as the **Compute Engine default service account**.

13. Click the **Create** button at the bottom of the page to deploy the image to Cloud Run and wait for the deployment to finish. A green check mark will appear next to the deployed service name when it is ready.

14. Click the **Logs** tab to verify everything is working fine. Wait a few minutes until you see the message `Appsmith is running` in the logs.

15. Click the displayed URL link to open the unique and stable endpoint of the Appsmith instance.
...
16. Enter the details on the signup form to create an Appsmith account.
