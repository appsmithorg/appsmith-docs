...
4. When the new instance creation is complete, note down the **File share name** and **IP address** to use later when mounting the Filestore in Cloud Run container.  
...
9. Click the **Container** tab.
    ...
    | **APPSMITH_MONGODB_URI** | Specify the connection string of the external MongoDB instance as recommended in the [Prerequisites](#prerequisites) section.<br/> <i>Format</i>: <br/>`mongodb://<USERNAME>:<PASSWORD>@<HOST>:<PORT>/?authSource=<DATABASE_NAME>&replicaSet=<REPLICA_SET_NAME>[&options]`|
