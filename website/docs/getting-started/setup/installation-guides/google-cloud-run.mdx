...
5. If you want scalable, secure, and highly available services, enable the [Memorystore for Redis](https://console.cloud.google.com/apis/dashboard) API.
...
This guide is optional and you can skip to [Deploy Appsmith on Cloud Run](#deploy-appsmith-on-cloud-run). Follow the steps below if you want scalable, secure, and highly available services. Ensure that you have enabled the **Memorystore for Redis** API as mentioned in step 5 of the [Before you begin](#before-you-begin) section.
...
<figure>
    <img src="/img/redis-primary-endpoint.png" style= {{width:"100%", height:"auto"}} alt="Primary Endpoint"/>
    <figcaption align = "center"><i>Primary Endpoint</i></figcaption>
</figure>
...
3. In the form, select **Deploy one revision from an existing container image**.
    - In the **Container image URL**, enter:
       - For Community edition - `docker.io/appsmith/appsmith-ce`.
       - For Enterprise edition - `docker.io/appsmith/appsmith-ee`.
...
9. Click the **Container** tab.
    ...
    | **APPSMITH_ENCRYPTION_SALT** | Specify a encryption salt to encrypt values in database.|
    | **APPSMITH_ENCRYPTION_PASSWORD** | Specify a encryption password to encrypt values in database.|
    | **APPSMITH_REDIS_URL** | If you followed the steps in the [Create Redis Instance](#create-redis-instance) section, add the host and port from the Primary Endpoint of the Redis instance you noted down in step 5. <br/> <i>Format</i>: <br/>`redis://<HOST>:<PORT>`|
...
13. Click the **Logs** tab to verify everything is working fine. Wait a few minutes until you see the message `Appsmith is running` in the logs.
