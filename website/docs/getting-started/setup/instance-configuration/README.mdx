...
Change the parameter values in the `values.yaml` file and run the below command:
  ...
  ```
  For example, the below command sets the `APPSMITH_SIGNUP_DISABLED` variable to `true` and configures it to disable the signup button on the Appsmith login screen.
