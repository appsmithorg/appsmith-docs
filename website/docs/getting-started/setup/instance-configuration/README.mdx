...
| **Session Timeout** | General | Set the duration after which inactive users are automatically logged out. The minimum time you can set is 1 minute, and the maximum is 30 days, in `DD:HH:MM` format. This feature is available to enterprise users.|
