---
description: This page provides step-by-step instructions for configuring your Appsmith instance using environment variables, tailored to Docker, Helm, and ECS installations.
---
...
3. Change the parameter values in the `values.yaml` file available under the `applicationConfig` section. For example, change the email address from which your messages are sent, as shown below:
