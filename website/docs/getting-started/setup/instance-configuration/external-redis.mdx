...
2. Retrieve the connection details of your Redis instance, including the hostname and port. If authentication is enabled, also retrieve the password. Store these details securely. You need them to configure the Redis instance on Appsmith. 
...
2. Add or update the following environment variables with your Redis connection details:  
   - `APPSMITH_REDIS_URL=redis://:{password}@{redis.host}:{port}`  

   Replace:
   - `{password}` with the Redis password (leave blank if no authentication is required).  
   - `{redis.host}` with the Redis instance hostname or IP address.  
...
- [Configure External MongoDB](/getting-started/setup/instance-configuration/external-mongodb): Learn how to set up an external MongoDB instance.