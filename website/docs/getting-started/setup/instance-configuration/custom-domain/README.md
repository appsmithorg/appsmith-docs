...
1. Rename the certificate file as `fullchain.pem` and the key file as `privkey.pem`.
   ...
   :::caution Password-protected keys are not supported
   Appsmith uses Caddy as its internal reverse proxy, and Caddy cannot load password-protected (encrypted) private keys. If your key file is password-protected, you must remove the passphrase before proceeding:
   ```bash
   openssl pkey -in privkey.pem -out privkey-decrypted.pem
   mv privkey-decrypted.pem privkey.pem
   ```
   If you skip this step, Caddy will fail to start and Appsmith will not be accessible over HTTPS.
   :::
...
2. Verify that the private key file is valid by running the following command in the terminal:
   ...
   ```
   If the key is still password-protected, you will be prompted for the pass phrase. If the command runs without prompting for a password, the key is ready to use. Take a note of the hash generated by the command.
