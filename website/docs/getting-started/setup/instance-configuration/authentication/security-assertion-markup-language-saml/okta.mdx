...
* **Name:** Enter the attribute name in the desired format (e.g.,  `firstName`, `lastName`, `email`). Copy the claim name for the newly created claim to add it later in the SAML configurations in Appsmith.
...
87. On your application's homepage, go to the **Sign-on tab** and copy the **Metadata** URL from the **SAML 2.0** section to add it later in the SAML configurations in Appsmith.
