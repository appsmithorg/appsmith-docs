...
Indicates the port number on which the email server is listening. The port number depends on the email server's setup and the protocol it uses. For example, port 25 is commonly used for unencrypted transport, port 587 for submission with StartTLS, and port 465 for SMTPS with SSL. Notably, port 465 is commonly used for Gmail. Choose the appropriate port that corresponds to your email server's setup and protocols.
