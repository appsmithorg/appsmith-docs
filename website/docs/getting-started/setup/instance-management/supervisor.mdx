...
Appsmith uses Supervisor to manage and monitor key subsystems, including the Appsmith server, Caddy, and MongoDB. If a subsystem becomes unresponsive, Supervisor attempts to restart it automatically. If the [Health Check API](/getting-started/setup/instance-management/API-Referenceh) fails, you can use Supervisor to diagnose and manage subsystems.
