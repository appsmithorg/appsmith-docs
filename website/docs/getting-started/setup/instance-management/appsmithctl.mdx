...
```
The backup archive file is stored in the `/appsmith-stacks/connect-data/backup/` folder. You can access it on the host machine at `./stacks/connect-data/backup/.`
...
docker cp appsmith:/appsmith-stacks/connect-data/backup/appsmith-backup-DATE_AND_TIMESTAMP.tar.gz .
```
...
```
The archive file is stored in the container directory `/appsmith-stacks/connect-data/backup/`.

4. Copy the archive file
```bash
# Replace ANY_APPSMITH_POD_NAME with the pod name
#highlight-next-line
kubectl cp ANY_APPSMITH_POD_NAME:/appsmith-stacks/connect-data/backup/appsmith-backup-TIMESTAMP.tar.gz appsmith-backup-TIMESTAMP.tar.gz
```
...
```bash
docker cp appsmith:/appsmith-stacks/connect-data/backup/appsmith-data.archive .
```
This command copies the backup archive in your current directory.
...
# Replace `ANY_APPSMITH_POD_NAME` with the pod name
#highlight-next-line
kubectl cp ANY_APPSMITH_POD_NAME:/appsmith-stacks/connect-data/backup/appsmith-data.archive appsmith-data.archive
```
...
# Replace `ANY_APPSMITH_POD_NAME` with the pod name
#highlight-next-line
kubectl cp appsmith-backup-TIMESTAMP.tar.gz ANY_APPSMITH_POD_NAME:/appsmith-stacks/connect-data/backup/
```
...
```bash
docker cp ./appsmith-data.archive appsmith:/appsmith-stacks/connect-data/restore/
```
...
# Replace `ANY_APPSMITH_POD_NAME` with the pod name
#highlight-next-line
kubectl cp ./appsmith-data.archive ANY_APPSMITH_POD_NAME:/appsmith-stacks/connect-data/restore/
```
