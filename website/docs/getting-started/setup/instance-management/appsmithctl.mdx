...
<TabItem label="Community Edition" value="communityEdition"> 
  ...
  The output file is stored in the container directory `/appsmith-stacks/data/backup/appsmith-data.archive`.
  ...
  ```bash
  kubectl cp appsmith:/appsmith-stacks/configuration/docker.env .
  ```
...
<TabItem label="Community Edition" value="communityEdition"> 
  ...
  * Copy the `docker.env` file using the below command:
  ...
  ```bash
  kubectl cp ./docker.env appsmith-0:/appsmith-stacks/configuration/
  ```
...
<TabItem label="Business Edition" value="businessEdition"> 
  ...
  * Copy the `docker.env` file using the below command:
  ...
  # if autoscaling is turned off, use appsmith-0 in place of <ANY-APPSMITH-POD-NAME>
  #highlight-next-line
  kubectl cp ./docker.env <ANY-APPSMITH-POD-NAME>:/appsmith-stacks/configuration/
  ```
...
```
4. If autoscaling is turned on then restart with:
```bash
