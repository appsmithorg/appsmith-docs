...
```
The backup archive is stored in the container directory `/appsmith-stacks/data/backup/`.
...
docker-compose exec appsmith cat /appsmith-stacks/configuration/docker.env | grep ENCRYPTION
```
Keep these details handy as these are needed in case you want to restore the Appsmith database.
