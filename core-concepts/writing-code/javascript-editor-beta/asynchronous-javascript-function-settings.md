...
* Add a [table](../../../reference/widgets/table.md) widget (_Table1_), and add a function in `JSObject5` that calls an API _`getUsers`_. The _`getUsers`_ API call is embedded in the _`showData`_ function and generates a user listing.

![Call a getUsers API in the showData JS function](<../../../.gitbook/assets/Bind-Response-to-widget (1).png>)

* Bind the response generated by `showData` to table widget `Table1` by adding the below code in the `Table Data` property available on the property pane of the [Table](../../../reference/widgets/table.md#table-data) widget.
